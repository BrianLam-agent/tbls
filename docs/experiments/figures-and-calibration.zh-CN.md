...
本页解释 TBLS 消融实验中一个反复出现的"怪图"——PR 曲线上近乎垂直的
悬崖——背后的数学，并给出一个自包含的复现器。当 PR 或 ROC 图上某处
看着病态、你想知道是不是 bug 时读它。它不是用户指南；CLI 见
[cli-visualize.md](cli-visualize.md)，流水线总入口见 [index.md](index.md)，
可跑教程见 [`../../examples/README.md`](../../examples/README.md)。

## "PR 悬崖"症状

在 `biomedical_larger` 的几个 cohort（`BC`、`CG`、`CKD`、`DM`）上，
`pr_{cohort}.png` 中 TBLS 变体曲线（纯 `TBLS`、`TBLS IFS`、`TBLS Graph`、
`TBLS Full`）出现近乎垂直的"悬崖"——精确率在某个 recall 处骤降，之后
曲线贴在一起下沉到流行率。

经验值（4 个 TBLS 变体，`biomedical_larger`，cohort `CG`）：

| cohort | 悬崖 recall（约） | 悬崖前精确率 | 悬崖底精确率 | 正类流行率 |
|---|---|---|---|---|
...
这不是绘图 bug，不是输入错（我们验证过 `y_score` 是真实的 `predict_proba`
输出，不是 `y_pred`），也不是折拼接 bug（用的是 predict+concat+单次
`precision_recall_curve`，不是曲线坐标拼接）。它是 TBLS 如何产出
`predict_proba` 的后果——没有任何概率标定步骤。

## 为什么 TBLS 会产出 `0.5` 概率平台
...
`predict_proba` 是 `softmax(Z_out)`（二分类即 one-vs-rest 等价），一个
样本的正类概率为
...
cohort，这会**成批**发生，原因有二：

1. **ridge 解出的 `W` 让 `A_enhᵀ A_enh` 最小特征值方向被
   `(σ_k + λ)⁻¹` 缩放**。训练能量小的方向对 `Δ W` 贡献小幅度；ridge
   故意选"最温和"的 `W` 仍能拟合 `Y`，这恰恰把许多"模糊"样本推到近零
   `Δ z`。这些样本的 `p_i` 落在 `0.5` 附近的窄带。

2. **基于树的特征映射 `A_enh` 已经把许多相似样本映射到增强空间中的邻近
   点**，所以一个 cohort 的边界样本（训练折上既不确信正也不确信负的）
   都落在同一个 `Δ W · A_enh ≈ 0` 流形附近。
...
（`argmin Σ log(1 + exp(-y_i w·x_i))`），凸拟合把确信样本推到大幅度
`w·x`、边界样本推到 `0` 附近的平滑尾——没有 ridge 的"温和 W 投影"。
它的 `predict_proba` 跨所有测试样本从近 `1` 连续走到近 `0`；阈值扫描
永远不跨密度平台，所以没有悬崖。

## 为什么悬崖是精确率-召回率专属伪影

`precision_recall_curve` 从高分到低分走阈值；每个阈值处
`precision = TP / (TP + FP)`。当阈值在 `0.5` 平台之上时，488 个平台
样本还没"预测为正"，精确率只跟那些真正高分的正样本走，保持中等。阈值
一跌破平台，**全部 488 个样本同时变成"预测为正"**；其中只有约流行率
...
recall 跳约 95（平台里的正样本），而精确率分母涨 488——比值骤降。这就
是悬崖。悬崖底恰好是：

```
"阈值之上全预测正"时测试集里正占比 ≈ 流行率
precision → TP / N_total = N_pos / N_total = 流行率    当 recall → 1
```

与 `BC`/`CG`/`CKD` 上的 `≈ 0.195` 底、`DM` 上的 `≈ 0.25` 底吻合
（细微差异来自每折流行率，因为 `visualize.py` 算曲线前先拼接折）。

ROC 曲线受影响小得多，因为它的轴是 `(FPR, TPR)` = `(FP/N_neg, TP/N_pos)`，
两者都随阈值下降只增不减——加 488 样本按各自比例增长两者，ROC 点沿对角
滑而非垂直掉。ROC 积分（`auroc`）对任何严格单调的得分重塑不变，这就是
为什么在本数据集上 `auroc` 比 `auprc` 是更稳定的 TBLS-vs-LR 判别量。
...
标定——**未来工作项，当前未实现**，且它属于估计器契约（`BaseEstimator`
估计器 + 标定器包装）而非本实验流水线。

## 复现器

自包含复现器（不画图，只给数字），验证平台存在并驱动悬崖：
...
- `train.py` 把 `y_score = model.predict_proba(X_te)` 以 `float32`、形状
  ...
  `int64` 存。两者作为独立数组，不会混淆。
- `visualize.py::_cohort_pr` 在收到 2-D `y_score` 时切片 `y_score[:, 1]`，
  拼接折后**恰好调一次** `precision_recall_curve(y_true, y_score)`，且
  绝不在折内重新归一化得分。实现匹配 `all_y_true.extend(y_test); ...
  extend(y_score); precision_recall_curve(all_y_true, all_y_score)` 配方。
- `.npz` 仅当 `grid_point is None` 时由 `_cross_validate` 写，所以纯
  train 运行有原始预测（ROC/PR/混淆图可画）；网格运行不产 side-file
  （体积），`visualize.py` 对它们跳过那三张图并在终端提示。

若未来加了标定器包装，标定运行的 PR 曲线上悬崖应消失，`auprc` 会朝
`auroc` 上升；`TBLS` 未标定 `predict_proba` 的实现行为会同样作为测试
夹具契约保留。
