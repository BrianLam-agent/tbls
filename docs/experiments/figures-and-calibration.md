...
`TBLS` ablation experiments — a near-vertical cliff on the PR curves — and
provides a self-contained reproducer. Read it when something on a PR or ROC
plot looks pathological and you want to know whether it is a bug. It is not a
user guide; for the CLI see [cli-visualize.md](cli-visualize.md), for the
overall pipeline entry [index.md](index.md), for the runnable tutorial
